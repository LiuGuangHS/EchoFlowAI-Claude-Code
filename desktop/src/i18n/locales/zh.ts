...
export const zh: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': '直接运行宿主机命令，用于安装插件、Skills、MCP 等扩展。桌面端已内置 echoflow-code；文档里的 claude <参数> 可替换成 echoflow-code <参数>，例如 echoflow-code plugin install ... 或 echoflow-code mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '只会清理：echoflow-code-open-tabs、echoflow-code-session-runtime、echoflow-code-theme、echoflow-code-locale、echoflow-code-app-zoom 和 echoflow-code.persistence.schemaVersion。',
  ...
  'settings.echoflowAPIOfficialLogin.connectModeTitle': '接入方式',
  'settings.echoflowAPIOfficialLogin.connectModeManagementToken': '系统访问令牌（推荐）',
  'settings.echoflowAPIOfficialLogin.connectModeManualKey': '已有调用 Key（兼容）',
  'settings.echoflowAPIOfficialLogin.manualKeyDesc': '仅支持基础模型调用，不支持余额查询和自动获取模型列表',
  'settings.echoflowAPIOfficialLogin.managementTokenPlaceholder': '粘贴系统访问令牌',
  'settings.echoflowAPIOfficialLogin.validateToken': '检测令牌',
  'settings.echoflowAPIOfficialLogin.validating': '检测中...',
  'settings.echoflowAPIOfficialLogin.balance': '余额',
  'settings.echoflowAPIOfficialLogin.userGroup': '用户分组',
  'settings.echoflowAPIOfficialLogin.selectMainModel': '主模型（main）',
  'settings.echoflowAPIOfficialLogin.selectHaikuModel': '快速模型（haiku）',
  'settings.echoflowAPIOfficialLogin.modelFetchFailed': '模型列表获取失败，将使用默认模型',
  'settings.echoflowAPIOfficialLogin.goRegister': '注册 EchoFlowAPI',
  'settings.echoflowAPIOfficialLogin.goGetToken': '生成系统访问令牌',
  'settings.echoflowAPIOfficialLogin.tokenInvalid': '令牌无效或已过期，请重新生成',
  'settings.echoflowAPIOfficialLogin.serviceUnavailable': '暂时无法连接 EchoFlowAPI，请稍后重试',
  'settings.echoflowAPIOfficialLogin.compatibilityNote': '当前版本会暂时使用系统访问令牌完成调用配置；待子调用 Key 接口稳定后，将改为自动创建并保存子调用 Key。',
  'settings.echoflowAPIOfficialLogin.managementTokenGuide': '请前往清云 API 后台 → 个人设置 → 安全设置 → 系统访问令牌 → 生成令牌，复制后粘贴至此',
  'settings.echoflowAPIOfficialLogin.manualKeyPlaceholder': '输入调用 Key / 应用码',
  'settings.echoflowAPIOfficialLogin.mainModelPlaceholder': '主模型 ID（必填）',
  'settings.echoflowAPIOfficialLogin.haikuModelPlaceholder': '快速模型 ID（留空同主模型）',
  ...
  'settings.providers.echoflowAPIOfficialName': 'EchoFlowAPI（清云 API）',
  'settings.providers.echoflowAPIOfficialDesc': '官方一站式接入 500+ 模型，支持 Claude / OpenAI 兼容协议，国内访问稳定',
  ...
  'settings.providers.settingsJsonDesc': '<EchoFlow AppData>/echoflow/settings.json — 直接编辑，保存时写入。',
  ...
  'settings.general.storagePortableDirPlaceholder': '选择一个用于保存 EchoFlow Code 数据的文件夹',
  ...
  'settings.general.storageMoveHint': '切换目录不会自动搬迁旧数据。如果希望旧会话继续出现，请把原目录下的 projects、skills、plugins、echoflow 等数据复制到新目录。要做便携包，建议把目录放在应用旁边并一起压缩。',
  ...
  'settings.general.legacyMigrationTitle': '从旧版本导入数据',
  'settings.general.legacyMigrationDescription': '检查旧版本配置并复制到当前 EchoFlow 数据目录；不会覆盖当前数据，也不会删除旧数据。',
  'settings.general.legacyMigrationCheck': '检查旧版本数据',
  'settings.general.legacyMigrationImport': '导入旧版本数据',
  'settings.general.legacyMigrationReadyCount': '可导入 {count} 项',
  'settings.general.legacyMigrationExistsCount': '已存在 {count} 项',
  'settings.general.legacyMigrationMissingCount': '未找到 {count} 项',
  'settings.general.legacyMigrationFailedCount': '异常 {count} 项',
  'settings.general.legacyMigrationConfirmTitle': '导入旧版本数据？',
  'settings.general.legacyMigrationConfirmBody': '将从旧版本目录复制配置到当前 EchoFlow 数据目录，不会删除旧数据，不会覆盖当前已有数据。',
  'settings.general.legacyMigrationSuccess': '已导入旧版本数据。新会话将使用导入后的 provider 配置，已有运行中的会话不受影响。',
  'settings.general.legacyMigrationFailed': '旧版本数据导入失败：{error}',
  'settings.general.legacyMigrationNoData': '未发现可导入的旧版本数据。',
  ...
  'settings.general.notificationsTestTitle': 'EchoFlow Code 通知已启用',
  ...
  'settings.general.traceHintOn': '新会话会把精简请求、响应、状态事件写入本机 traces 目录。',
