...
export const jp: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': 'プラグイン、スキル、MCP のセットアップ用にホストマシンのコマンドを実行します。デスクトップアプリには echoflow-code が含まれています。ドキュメントの claude <args> を echoflow-code <args> に置き換えてください。例: echoflow-code plugin install ... や echoflow-code mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': 'クリアされるのは次のみ: echoflow-code-open-tabs、echoflow-code-session-runtime、echoflow-code-theme、echoflow-code-locale、echoflow-code-app-zoom、echoflow-code.persistence.schemaVersion。',
  ...
  'settings.echoflowAPIOfficialLogin.connectModeTitle': '接続方式',
  'settings.echoflowAPIOfficialLogin.connectModeManagementToken': '管理トークン（推奨）',
  'settings.echoflowAPIOfficialLogin.connectModeManualKey': '既存の API キー（互換）',
  'settings.echoflowAPIOfficialLogin.manualKeyDesc': '基本モデル呼び出しのみ対応 — 残高確認・モデル一覧取得は不可',
  'settings.echoflowAPIOfficialLogin.managementTokenPlaceholder': '管理トークンを貼り付け',
  'settings.echoflowAPIOfficialLogin.validateToken': 'トークンを検証',
  'settings.echoflowAPIOfficialLogin.validating': '検証中...',
  'settings.echoflowAPIOfficialLogin.balance': '残高',
  'settings.echoflowAPIOfficialLogin.userGroup': 'ユーザーグループ',
  'settings.echoflowAPIOfficialLogin.selectMainModel': 'メインモデル（main）',
  'settings.echoflowAPIOfficialLogin.selectHaikuModel': '高速モデル（haiku）',
  'settings.echoflowAPIOfficialLogin.modelFetchFailed': 'モデル一覧の取得に失敗しました。デフォルトモデルを使用します',
  'settings.echoflowAPIOfficialLogin.goRegister': 'EchoFlowAPI に登録',
  'settings.echoflowAPIOfficialLogin.goGetToken': '管理トークンを取得',
  'settings.echoflowAPIOfficialLogin.tokenInvalid': 'トークンが無効または期限切れです。再生成してください',
  'settings.echoflowAPIOfficialLogin.serviceUnavailable': '現在 EchoFlowAPI に接続できません。しばらくしてからお試しください。',
  'settings.echoflowAPIOfficialLogin.compatibilityNote': '現在のバージョンでは管理トークンを一時的に呼び出し設定に使用します。プロジェクトキー作成 API が安定したら、スコープ付きプロジェクトキーを自動作成して保存します。',
  'settings.echoflowAPIOfficialLogin.managementTokenGuide': 'EchoFlowAPI ダッシュボード → プロフィール → セキュリティ → 管理トークン → 生成 でトークンを作成してコピーし、ここに貼り付けてください',
  'settings.echoflowAPIOfficialLogin.manualKeyPlaceholder': 'API キー / アプリコードを入力',
  'settings.echoflowAPIOfficialLogin.mainModelPlaceholder': 'メインモデル ID（必須）',
  'settings.echoflowAPIOfficialLogin.haikuModelPlaceholder': '高速モデル ID（空白にするとメインと同じ）',
  ...
  'settings.providers.echoflowAPIOfficialName': 'EchoFlowAPI（清雲 API）',
  'settings.providers.echoflowAPIOfficialDesc': '500以上のモデルへの公式ゲートウェイ — Claude / OpenAI 互換、中国国内アクセス安定',
  ...
  'settings.providers.settingsJsonDesc': '<EchoFlow AppData>/echoflow/settings.json — 直接編集できます。保存時に書き込まれます。',
  ...
  'settings.general.storagePortableDirPlaceholder': 'EchoFlow Code データ用のフォルダを選択',
  ...
  'settings.general.storageMoveHint': 'ディレクトリを切り替えても既存のデータは移行されません。以前のセッションを引き続き表示したい場合は、古いディレクトリから projects、skills、plugins、echoflow および関連フォルダをコピーしてください。ポータブルバンドルにする場合は、このフォルダをアプリの隣に置いて一緒に zip 化してください。',
  ...
  'settings.general.legacyMigrationTitle': '旧バージョンからデータをインポート',
  'settings.general.legacyMigrationDescription': '旧バージョンの設定を確認し、現在の EchoFlow データディレクトリへコピーします。現在のデータは上書きされず、旧データも削除されません。',
  'settings.general.legacyMigrationCheck': '旧データを確認',
  'settings.general.legacyMigrationImport': '旧データをインポート',
  'settings.general.legacyMigrationReadyCount': '{count} 件をインポート可能',
  'settings.general.legacyMigrationExistsCount': '{count} 件は既に存在',
  'settings.general.legacyMigrationMissingCount': '{count} 件は未検出',
  'settings.general.legacyMigrationFailedCount': '{count} 件に異常',
  'settings.general.legacyMigrationConfirmTitle': '旧データをインポートしますか？',
  'settings.general.legacyMigrationConfirmBody': '旧バージョンのディレクトリから現在の EchoFlow データディレクトリへ設定をコピーします。旧データは削除されず、現在の既存データも上書きされません。',
  'settings.general.legacyMigrationSuccess': '旧バージョンのデータをインポートしました。新しいセッションはインポートされた provider 設定を使用し、実行中のセッションには影響しません。',
  'settings.general.legacyMigrationFailed': '旧データのインポートに失敗しました: {error}',
  'settings.general.legacyMigrationNoData': 'インポート可能な旧データは見つかりませんでした。',
  ...
  'settings.general.notificationsTestTitle': 'EchoFlow Code の通知が有効になりました',
